...
export function CreateConceptForm({ topicId, topicName, onSuccess }: CreateConceptFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { topic_id: string; name: string; description: string }) =>
      hierarchyApi.createConcept(data),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      const message = error.response?.data?.error || 'Failed to create concept';
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Add a concept under <span className="font-semibold text-gray-900">{topicName}</span>
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
            label="Concept Name"
            placeholder="e.g., Solving Linear Equations, Conduction"
            value={formData.name}
